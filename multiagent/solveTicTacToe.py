...
class TicTacToeAgent():
    ...
    def getAction(self, gameState, gameRules):
        ...
        for action in actions:
            values.append(self.evaluationFunction(gameState.generateSuccessor(action).boards, gameRules))
    ...
    def rotateBoard(self, board):
        ...
        for i in range(3):
            newboard = list(board)
            newboard[0] = originBoard[6]
            newboard[1] = originBoard[3]
            newboard[2] = originBoard[0]
            newboard[3] = originBoard[7]
            newboard[5] = originBoard[1]
            newboard[6] = originBoard[8]
            newboard[7] = originBoard[5]
            newboard[8] = originBoard[2]
            originBoard = list(newboard)
    ...
    def reflectBoard(self, board):
        ...
        newboard = list(board)
        for i in range(3):
            newboard[i] = board[i+6]
            newboard[i+6] = board[i]
        boardsList.append(newboard)
        newboard = list(board)
        for i in range(0,7,3):
            newboard[i] = board[i+2]
            newboard[i+2] = board[i]
        boardsList.append(newboard)
        newboard = list(board)
        newboard[0] = board[8]
        newboard[8] = board[0]
    ...
    def fingerPrint(self, board, gameRules):
        ...
        patterns = pattern_1 + pattern_a + pattern_b + pattern_c + pattern_d + pattern_ab + pattern_ad + pattern_c2
        rotate_board = self.rotateBoard(board)
        reflec_board = self.reflectBoard(board)
        trans_board = [board for board in rotate_board+reflec_board if board in patterns][0]

        if trans_board in pattern_1:
            return {'1':1}
        elif trans_board in pattern_a:
            return {'a':1}
        elif trans_board in pattern_b:
            return {'b':1}
        elif trans_board in pattern_c:
            return {'c':1}
        elif trans_board in pattern_d:
            return {'d':1}
        elif trans_board in pattern_ab:
            return {'a':1, 'b':1}
        elif trans_board in pattern_ad:
            return {'a':1, 'b':1}
        elif trans_board in pattern_c2:
            return {'c':2}
        else:
            return False

    def evaluationFunction(self, board, gameRules):
        win_cond = [{'a':1}, {'b':2}, {'c':2}, {'b':1, 'c':1}]
        fps = [self.fingerPrint(board[i], gameRules) for i in range(3)]
        result = {}
        for fp in fps:
            for k in fp:
    ...
    # def minimax(gameState, gameRules, agent, depth):
    #     result=[]

    #     if gameRules.isGameOver(gameState.boards):
    #         return 2,''
    #     elif depth == 2:
    #         return (evaluationFunction(gameState, gameRules, agent),0)

    #     if agent == 0: #AI
    #         result=[-1e10,'']
    #         for a in gameState.getLegalActions(gameRules):
    #             s = gameState.generateSuccessor(a)
    #             newResult=minimax(s,gameRules, 1,depth)
    #             oldResult = result[0]
    ...
    #         for a in gameState.getLegalActions(gameRules):
    #             s = gameState.generateSuccessor(a)
    #             newResult=minimax(s,gameRules,0,depth+1)
    #             oldResult = result[0]
    #             if newResult[0] < oldResult:
    #                 result[0]=newResult[0]
    #                 result[1]=a
    ...
    # def evaluationFunction(gameState, gameRules, agent):
    ...
    # return minimax(gameState, gameRules, 0, 0)[1]
