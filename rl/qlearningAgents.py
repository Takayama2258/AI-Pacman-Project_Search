...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        "*** YOUR CODE HERE ***"
        result = None
        optVal = self.computeValueFromQValues(state)
        for action in self.getLegalActions(state):
          if self.getQValue(state, action) == optVal:
            result = action
        return result
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in f.keys():
          qVal += self.weights[feature] * f[feature]
        return qVal
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature in f.keys():
          self.weights[feature] = self.weights[feature] + self.alpha * diff * f[feature]
